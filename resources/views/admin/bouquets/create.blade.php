...
@section('title', 'Create Bouquet')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gh-text">Create Bouquet</h1>
    </div>
    <div class="bg-gh-bg-secondary border border-gh-border rounded-lg p-6">
        <form method="POST" action="{{ isset($bouquets) ? route('admin.bouquets.update', $bouquets) : route('admin.bouquets.store') }}">
            @csrf
            @if(isset($bouquets))
                @method('PUT')
            @endif
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <a href="{{ route('admin.bouquets.index') }}" class="px-4 py-2 bg-gh-bg-tertiary text-gh-text rounded-lg hover:bg-gh-border">Cancel</a>
